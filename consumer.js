...
npWorker.worker.on('message', (msg, next, msgid) => {
  m.emit('broadcast', msg);
  next();
});

// Listener for taken job entries to delete from list
jWorker.worker.on('message', (msg, next, msgid) => {
  m.emit('delete', msg);
  next();
});
...
app.get('/entries', (req, res) => {
  ...
  let msgs = [];
  let pWorker = new Worker(cfg.queue.p.name, cfg.queue.p.settings);

  pWorker.size((err, size) => {
    pWorker.start();
    pWorker.worker.on('message', (msg, next, msgid) => {
      let item = {...JSON.parse(msg), ...{id: msgid}};
      msgs.push(item);
...
app.post('/jobs', (req, res) => {
  ...
  rsmq.deleteMessage({ qname: cfg.queue.p.name, id: req.body.id }, (err, resp) => {
    ...
    if (resp === 1) {
      ...
      rsmq.sendMessage({ qname: cfg.queue.j.name, message: JSON.stringify({del: req.body.id}) }, (err, resp) => {
